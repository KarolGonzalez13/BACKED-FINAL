...
async function buscarProductosPorNombre(termino) {
    const productosRef = await productoDB.get(); // Obtén todos los productos
    const productos = [];

    productosRef.forEach((doc) => {
        const producto = { id: doc.id, ...doc.data() };
        if (producto.nombre && producto.nombre.toLowerCase().includes(termino.toLowerCase())) {
